...
private:
  ...
  SDNode *SelectConditionalBranchForTriCore(SDNode *N);
...
SDNode *LEGDAGToDAGISel::SelectMoveImmediate(SDNode *N) {
  ...
  if ((ImmVal & SupportedMask) != ImmVal) {
  	outs() <<" LEG : Immediate size not supported!\n";
    return SelectCode(N);
  }
...
SDNode *LEGDAGToDAGISel::SelectConditionalBranch(SDNode *N) {
	...
	//return SelectConditionalBranchForTriCore(N);
  ...
  //SDNode *Compare = CurDAG->getMachineNode(LEG::CMP, N, MVT::Glue, LHS, RHS, Chain);
  ...
  MachineSDNode *branchInst = CurDAG->getMachineNode(LEG::Bcc, N, MVT::Other, BranchOps);
...
SDNode *LEGDAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::FrameIndex: {
    	...
    	return CurDAG->getMachineNode(LEG::ADDri, dl, MVT::i32, TFI,
